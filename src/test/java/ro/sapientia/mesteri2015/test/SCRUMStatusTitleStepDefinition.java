...
public class SCRUMStatusTitleStepDefinition {
	...
	public void I_enter_in_the_title_textbox_and_I_push_the_add_button(
		...
		WebElement titleTextBox = driver.findElement(By.id("status-title"));
		titleTextBox.clear();
		titleTextBox.sendKeys(additionTerms);

		// Click on searchButton
		WebElement searchButton = driver.findElement(By.id("add-status-button"));
		searchButton.click();
	}

	@Then("^I should get result \"([^\"]*)\" in statuses list$")
	public void I_should_get_result_in_statuses_list(String expectedResult)
			throws Throwable {
		WebElement calculatorTextBox = driver.findElement(By.id("status-title"));
